...
function BoardPage() {
  ...
  return (
    <>
      ...
      <Tab defaultValue="소통" onClickTab={onClickTab} tabList={TAB_LIST} style="text-16 pb-2 px-2" />
      ...
      <div className="flex gap-2 flex-nowrap overflow-x-scroll scroll-hidden my-5">
        {CATEGORY_LIST.map((btn) => (
          <button
            ...
            key={btn}
            className={`mint_btn !font-normal !rounded-[32px] !px-4 !py-3 ${selected !== btn ? '!bg-Labels-Secondary' : null}`}
          >
            {btn}
          </button>
        ))}
      ...
      <div>
        {DATA.map(({ category, content, heart, host, id, save, title }) => (
          <PostCard
            ...
            title={title}
            id={id}
            cardStyle="!bg-background rounded-none border-b border-Line-Normal-Normal"
          />
        ))}
      </div>
    </>
