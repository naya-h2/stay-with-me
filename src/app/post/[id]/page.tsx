...
function PostPage() {
  const { category, commentList, content, created_At, heart, host, id, save, title } = DATA;
  return (
    <HeaderLayout>
      <p className="text-14 text-main-mint font-medium mt-8">{category}</p>
      ...
      <p className="text-14 font-medium">댓글 {commentList.length}</p>

      <div>
        {commentList
          ? commentList.map(({ content, created_At, heart, host, id }) => (
